...
- pureView：Help components have the ability to "better avoid re-render"
...
function App() {
  function btnClick() {
     ...
     *
     * 2、setState is a method of attaching to each store container generated by resy
     */
    // @example A
    store.setState({
      count: count++,
 ...
 * About the deepEqual parameter, because props belong to the props attribute passed in from the outside of the pureView converted component
 * Therefore, it is independent of the internal data status update control of pureView and more dependent on the external control
