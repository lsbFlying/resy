 ...
 * can serve as state for updating component rendering as long as they are accessed through useStore.
 * 2、Validate that function properties do not have the capacity to update components as state
 * if they are accessed directly through the store instead of being fetched from useStore.
 */
test("functionProp-useStore", async () => {
  type Store = {
    count: number;
    testFn?(): void;
    testFn2?(): void;
  ...
  const App = () => {
    const { count, testFn, testFn3, testFn4 } = useStore(store);
    const { testFn2 } = store;
