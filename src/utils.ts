...
export const storeErrorHandle = <S extends PrimitiveState>(store: S, fnName: "useStore" | "view") => {
  if (!store[REGENERATIVE_SYSTEM_KEY as keyof S]) {
    throw new Error(
      `The store parameter of ${fnName} is not a store created by resty's createStore!`
    );
  }
};
...
export const subscribeErrorHandle = <S extends PrimitiveState>(
  ...
  if (typeof listener !== "function") {
    throw new Error("The listening function of subscribe is not a function type!");
  }
  if (stateKeys !== undefined && toString.call(stateKeys) !== "[object Array]") {
    throw new Error("The listening dependencies attribute array of 'subscribe' is not array type data!");
  }
};
