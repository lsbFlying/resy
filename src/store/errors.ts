...
import { __REGENERATIVE_SYSTEM_KEY__, __STATE_REF_COUNTER_KEY__ } from "./static";
...
export const storeErrorProcessing = (
  ...
  if (
    __DEV__ && (
      (!(store as any)?.[__REGENERATIVE_SYSTEM_KEY__]) || (
        fnName === "useStore" && (
          (store as any)?.signals
          || (store as any)?.[__STATE_REF_COUNTER_KEY__]
        )
      )
    )
  ) {
  ...
  if (
    ...
    console.error(
      "The store generated by createSignals can be used directly in class components through store.signals,"
      + " without the need for additional connectStore to connect."
    );
  }
};
...
export const stateErrorProcessing = (params: {
  state: unknown;
  fnName?: "setState、syncUpdate" | "createStore" | "createSignals" | "useSignals" | "useConciseState";
  ...
  if (__DEV__ && stateType !== "Object") {
    ...
    const mode = options?.__mode__;

    let fnNameTemp = fnName;

    if (!fnNameTemp && ucs === true && mode === "signal") {
      fnNameTemp = "useSignals";
    }
    if (!fnNameTemp && ucs !== true && mode === "signal") {
      fnNameTemp = "createSignals";
    }
    if (!fnNameTemp && ucs !== true && mode === "state") {
  ...
  if (__DEV__ && (state as any)?.[__REGENERATIVE_SYSTEM_KEY__]) {
    console.error(
      ...
      + " createStore、createSignals that's has no update significance!"
    );
  }
};
