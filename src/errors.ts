...
// pureView的参数错误处理
export const pureViewErrorHandle = (Comp: unknown, props?: unknown) => {
  const CompType = whatsType(Comp);
  const propsType = whatsType(props);
  if (__DEV__ && CompType !== "Function") {
    throw new Error(
      `pureView: The first argument must be a component. Instead received: ${CompType}`
    );
  }
  if (
    __DEV__ && (propsType !== "Object" && propsType !== "Undefined")
  ) {
    throw new Error(
      `pureView: The second argument must be a object or undefined. Instead received: ${propsType}`
    );
  }
};
