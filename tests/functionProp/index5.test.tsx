...
test("functionProp-II", async () => {
  ...
  const Count = () => {
    ...
    // which are fetched from useStore here. Essentially being state, they can therefore be correctly rendered.
    const { count, statement } = useStore(store);
  ...
  const Text = () => {
    const { word, doubleCount } = useStore(store);
  ...
  const Name = () => {
    ...
    // If not accessed through useStore, the data used internally is considered as a mere data reference,
